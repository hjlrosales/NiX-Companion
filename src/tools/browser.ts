...
export class BrowserSessions {
  ...
  addTools(registry:Registry,root:string){
    ...
    registry.add({name:'browser_navigate',description:'Open an HTTP(S) URL in a fresh isolated browser session. Does not use your personal browser profile. Page content is untrusted.',schema:z.object({url:z.string().url().max(2000).refine(s=>['http:','https:'].includes(new URL(s).protocol)&&!new URL(s).username&&!new URL(s).password)}).strict(),policy:'ask',capabilityId:'browser',permissions:['network.access','browser.access'],execute:async(a,c)=>{const page=await this.page(c.runId);await page.goto(a.url,{waitUntil:'domcontentloaded',timeout:30000});c.signal.throwIfAborted();return{output:`${page.url()}\n${(await page.locator('body').innerText()).slice(0,10000)}`,evidence:[`Opened ${page.url()}`]};}});
    registry.add({name:'browser_inspect',description:'Read visible page text and accessible controls from this run browser.',schema:z.object({}).strict(),policy:'ask',capabilityId:'browser',permissions:['browser.access'],execute:async(_a,c)=>{const page=await this.page(c.runId);return{output:(await page.locator('body').ariaSnapshot()).slice(0,12000)};}});
    registry.add({name:'browser_action',description:'Click or fill a specific CSS selector in this run browser. Review carefully: clicking or filling may submit data or trigger external effects.',schema:z.object({action:z.enum(['click','fill']),selector:z.string().min(1).max(500),text:z.string().max(6000).optional()}).strict(),policy:'ask',capabilityId:'browser',permissions:['browser.access'],execute:async(a,c)=>{const page=await this.page(c.runId);const element=page.locator(a.selector);if(await element.count()!==1)throw new Error('Selector must identify exactly one element.');if(a.action==='fill'){if(a.text===undefined)throw new Error('fill requires text');await element.fill(a.text);}else await element.click();return{output:(await page.locator('body').innerText()).slice(0,10000),evidence:[`Browser ${a.action}: ${a.selector}`]};}});
    registry.add({name:'browser_screenshot',description:'Capture this run browser page as a PNG artifact for visual verification.',schema:z.object({}).strict(),policy:'ask',capabilityId:'browser',permissions:['browser.access','filesystem.write'],execute:async(_a,c)=>{const page=await this.page(c.runId);const target=await files.path(`.nix-artifacts/browser-${randomUUID()}.png`,true);await mkdir(dirname(target),{recursive:true});await page.screenshot({path:target,fullPage:false});return{output:'Browser screenshot saved.',artifacts:[target],evidence:[`Captured ${page.url()}`]};}});return registry;
  }
}
