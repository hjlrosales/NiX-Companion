...
export function documentTools(registry: Registry, root: string, project: string) {
  ...
  registry.add({name:'document_extract',description:'Extract source-linked text from PDF, DOCX, PPTX, text or scanned images. start is a zero-based section offset. Local OCR requires Tesseract.',schema:z.object({source:z.string().min(1).max(500),start:z.number().int().min(0).default(0),count:z.number().int().min(1).max(5).default(3)}).strict(),policy:'allow',capabilityId:'documents',permissions:['filesystem.read'],execute:(a,c)=>execute({...a,action:'extract'},c.signal)});
  registry.add({name:'document_create',description:'Create actual source-linked summary/report/slide deliverables with editable DOCX/PPTX, PDF and rendered previews. For a long source, deterministic extractive summarization preserves source sentences. Choose bundle for report and slides together. Source paths are workspace-relative. Uses installed Microsoft Word and PowerPoint for rendering on Windows.',schema:z.object({action:z.enum(['summary','report','slides','bundle']),source:z.string().max(500).optional(),title:z.string().min(1).max(110),sections:z.array(z.object({title:z.string().min(1).max(100),text:z.string().min(1).max(8000),reference:z.string().max(500).optional()}).strict()).max(50).optional()}).strict().refine(a=>!!a.source||!!a.sections?.length,'Supply source or sections'),policy:'ask',capabilityId:'documents',permissions:['filesystem.read','filesystem.write','process.execute'],execute:(a,c)=>execute(a,c.signal)});
  return registry;
}
